import { ListItemText, ListItem, List, Box, Container, Divider, Breadcrumbs, Link, Typography, IconButton, ListItemIcon } from '@mui/material';
...
function LinksPage() {
    ...
    function onDragEnd(result) {
        ...
        reorderLinks(selectedPage.id, organizedLinks);
    }
                            ...
                            {(provided, snapshot) => {
                                return (
                                    <List ref={provided.innerRef} {...provided.droppableProps}>
                                        { selectedPage.links.length > 0 && <Divider /> } 
                                        { selectedPage.links.map((link, index) => (
                                                    ...
                                                    <React.Fragment>
                                                        <ListItem
                                                            ...
                                                            sx={{
                                                                ...
                                                                width: '100%',
                                                                backgroundColor: snapshot.isDraggingOver && link.id === snapshot.draggingOverWith ? '#1DA1F2' : 'inherit',
                                                                color: snapshot.isDraggingOver && link.id === snapshot.draggingOverWith ? 'white' : 'inherit',
                                                                '&:hover': {
                                                                    backgroundColor: 'action.hover',
                                                                }
                                                            }}
                                                            onClick={() => {
                                                                dispatch({ type: 'SET_SELECTED_LINK', payload: link });
                                                        ...
                                                        >
                                                            ...
                                                            <ListItemIcon>
                                                                <SocialIcon url={`${link.url}`} bgColor="transparent" fgColor='black' style={{ height: 30, width: 30 }} />
                                                            </ListItemIcon>
                                                        </ListItem>
                                                        <Divider />
                                                    </React.Fragment>
